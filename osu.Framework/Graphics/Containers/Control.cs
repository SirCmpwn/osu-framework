...
{
    public partial class Control<T> : Drawable, IControl where T : Drawable
    {
        ...
        protected LifetimeList<T> children;
        ...
        protected Control(LifetimeList<T> lifetimeList = null)
        {
        ...
        protected void Add(IEnumerable<T> collection)
        {
            foreach (T d in collection)
                Add(d);
        }

        protected virtual bool Remove(T drawable)
        {
            if (drawable == null)
                return false;
        ...
        protected void Remove(IEnumerable<T> range)
        {
            if (range == null)
                return;
        ...
        protected virtual void Clear(bool dispose = true)
        {
        ...
        protected IEnumerable<T> AliveChildren => children.AliveItems;
        ...
        protected virtual void InvalidateFromChild(Invalidation invalidation, IDrawable source)
        {
        ...
        /// <param name="parentControl">The container whose children's DrawNodes to add.</param>
        ...
        private static void addFromControl(int treeIndex, ref int j, Control<T> parentControl, List<DrawNode> target, RectangleF maskingBounds)
        {
            List<T> current = parentControl.children.AliveItems;
            for (int i = 0; i < current.Count; ++i)
            {
                ...
                Control<T> control = drawable as Control<T>;
                if (control?.CanBeFlattened == true)
                {
                    // The masking check is overly expensive (requires creation of ScreenSpaceDrawQuad)
                    // when only few children exist.
                    control.IsMaskedAway = control.children.AliveItems.Count >= amount_children_required_for_masking_check &&
                        !maskingBounds.IntersectsWith(drawable.ScreenSpaceDrawQuad.AABBFloat);

                    if (!control.IsMaskedAway)
                        addFromControl(treeIndex, ref j, control, target, maskingBounds);
        ...
        protected internal override DrawNode GenerateDrawNodeSubtree(int treeIndex, RectangleF bounds)
        {
            ...
            int j = 0;
            addFromControl(treeIndex, ref j, this, target, childBounds);
        ...
        protected int IndexOf(T drawable)
        {
            return children.IndexOf(drawable);
        }

        protected bool Contains(T drawable)
        {
